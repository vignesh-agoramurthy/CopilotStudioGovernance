<template>
  <div class="taxonomy-index">
    <a
      v-for="item in items"
      :key="item.slug"
      :href="withBase(`/industries/${item.slug}/`)"
      class="taxonomy-card"
    >
...
import { industryIcons as icons } from "../../data/icons";

const items = industryData.filter(i =>
  missions.some(m => (m.industries ?? []).includes(i.slug))
);

function missionCount(slug: string): number {
  return missions.filter(m => (m.industries ?? []).includes(slug)).length;
}
</script>
